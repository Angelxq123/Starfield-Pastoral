...
/** Source-shaped steel helmet and brass feet, rendered with the shared native cuboid format. */
@SuppressWarnings({"null", "removal"})
@EventBusSubscriber(modid = StardewCraft.MODID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
public final class NativeMetalHeadRenderer extends EntityRenderer<MineMetalHeadEntity> {
    private static final String[] VARIANTS = {"metal_head"};
    private static volatile java.util.Map<String, NativeNpcModel> loaded = java.util.Map.of();
    private NativeNpcModel posedModel;
    private final java.util.Map<MineMetalHeadEntity,NativeMetalHeadPlayback> motions=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativeMetalHeadRenderer(EntityRendererProvider.Context context) { super(context); shadowRadius = .28F; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.METAL_HEAD.get(), NativeMetalHeadRenderer::new);
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativeMetalHeadRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native metal head model header");
            }
            ...
            for (String suffix : new String[]{"idle","walk","hit","death"}) {
                var clip = model.clips().get("animation.metal_head." + suffix);
    ...
    @Override protected boolean shouldShowName(MineMetalHeadEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(MineMetalHeadEntity entity) { return texture("metal_head"); }
    @Override public void render(MineMetalHeadEntity entity, float yaw, float partialTick, PoseStack stack,
                                 MultiBufferSource buffers, int light) {
        var model = loaded.get("metal_head");
        ...
        var motion=motions.computeIfAbsent(entity,e->new NativeMetalHeadPlayback(model));
        motion.sample((entity.level().getGameTime()+(double)partialTick)/20.,entity.moving(),
        ...
        for(var quad:model.quads()){
            ...
            for(var v:quad.vertices()){
                ...
                consumer.addVertex(stack.last().pose(),vertex.x,vertex.y,vertex.z).setColor((entity.color()>>16)&255,(entity.color()>>8)&255,entity.color()&255,255).setUv(v[3],v[4])
