...
record OfflineFarmCatchUpPlan(
    ...
    private static final Comparator<GlobalPos> POSITION_ORDER = Comparator
            .comparingInt((GlobalPos position) -> position.pos().getX())
            .thenComparingInt(position -> position.pos().getY())
            .thenComparingInt(position -> position.pos().getZ());
    ...
    static OfflineFarmCatchUpPlan create(
        ...
        List<GlobalPos> crops = retainAndSort(dimension, minX, maxX, minZ, maxZ, cropPositions);
        List<GlobalPos> trees = retainAndSort(dimension, minX, maxX, minZ, maxZ, treePositions);
        List<GlobalPos> sprinklers = retainAndSort(dimension, minX, maxX, minZ, maxZ, sprinklerPositions);
    ...
    private static List<GlobalPos> retainAndSort(
            ResourceKey<Level> dimension,
            int minX,
            int maxX,
            int minZ,
