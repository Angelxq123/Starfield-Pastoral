...
/** Approved Generic Bug, opaque signed shells followed by translucent membrane wings. */
@SuppressWarnings({"null", "removal"})
@EventBusSubscriber(modid = StardewCraft.MODID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
public final class NativeBugRenderer extends EntityRenderer<MineBugEntity> {
    private static final String[] VARIANTS = {"bug","armored_bug"};
    ...
    private final java.util.Map<MineBugEntity,NativeBugPlayback> motions=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativeBugRenderer(EntityRendererProvider.Context context) { super(context); shadowRadius = 0; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.ARMORED_BUG.get(), NativeBugRenderer::new);
        event.registerEntityRenderer(ModEntities.BUG.get(), NativeBugRenderer::new);
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativeBugRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native bug model header");
            }
            ...
            for (String suffix : new String[]{"fly","sway","hit","death"}) {
                var clip = model.clips().get("animation.bug." + suffix);
    ...
    @Override protected boolean shouldShowName(MineBugEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(MineBugEntity entity) { return texture(entity.variant()); }
    @Override public void render(MineBugEntity entity, float yaw, float partialTick, PoseStack stack,
        ...
        var motion=motions.computeIfAbsent(entity,e->new NativeBugPlayback(model));
        motion.sample((entity.level().getGameTime()+(double)partialTick)/20.,entity.hitTime(partialTick),
                entity.deathTime>0?(entity.deathTime+partialTick)/20.:0,entity.patrolFacing());
        var matrices=motion.pose().matrices();
        stack.pushPose();
        stack.translate(motion.swayX()*entity.getScale(),0,motion.swayZ()*entity.getScale());
        ...
        for (boolean translucent : new boolean[]{false,true}) {
            var consumer = buffers.getBuffer(translucent?(entity.armored()?NativeBugRenderTypes.ARMORED_MEMBRANE:NativeBugRenderTypes.MEMBRANE):RenderType.entityCutout(getTextureLocation(entity)));
            for (var quad : model.quads()) {
