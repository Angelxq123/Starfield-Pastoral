...
2026-09-15：为适配 MC 大地图，真实 NPC 的日常导航（包括日程、区域内走动及活动走位）
在上述角色基础速度上统一乘以 `NpcMotionProfile.TRAVEL_SPEED_MULTIPLIER=1.5`，保留相对快慢。
`speed` 仍表示原基础属性值，`travelSpeed()` 为实体实际应用值；旧存档实体加载后同样重新应用。
原生播放器将步态采样的世界步幅同步乘以 1.5，因此同样时间内走得更远、动画步频保持原有水平；
关键帧、动作幅度与起停混合不变，停止/传送仍按实际位移处理。
George 的推轮动作也补偿平移速度，轮胎和脚轮仍按真实路程滚动，原地转向保持原节奏。
剧情假人的显式移动速度、时长及其动画通路不参与此次日常移动倍率。
定向验证入口：`./gradlew -I compatibility/npc-travel-tests.gradle classes test`；
无窗口检查不代替实际地图里的观感验收，不为此启动额外游戏客户端。
本次编译通过，5 项定向测试全部通过：正式角色速度配置、多帧率下的新旧步频对照、
起停/离地/传送、轮椅前进后退时的推轮节奏与轮胎路程、原地转向及停下收势。
