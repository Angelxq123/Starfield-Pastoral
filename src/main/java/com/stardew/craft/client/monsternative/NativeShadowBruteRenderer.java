...
public final class NativeShadowBruteRenderer extends EntityRenderer<MineShadowBruteEntity> {
    private static final String[] VARIANTS = {"shadow_brute"};
    private static volatile java.util.Map<String, NativeNpcModel> loaded = java.util.Map.of();
    private NativeNpcModel posedModel;
    private final java.util.Map<MineShadowBruteEntity,NativeShadowBrutePlayback> motions=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativeShadowBruteRenderer(EntityRendererProvider.Context context) { super(context); shadowRadius = .28F; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.SHADOW_BRUTE.get(), NativeShadowBruteRenderer::new);
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativeShadowBruteRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native shadow brute model header");
            }
            ...
            for (String suffix : new String[]{"idle","walk","hit","death"}) {
                var clip = model.clips().get("animation.shadow_brute." + suffix);
    ...
    @Override protected boolean shouldShowName(MineShadowBruteEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(MineShadowBruteEntity entity) { return texture("shadow_brute"); }
    @Override public void render(MineShadowBruteEntity entity, float yaw, float partialTick, PoseStack stack,
                                 MultiBufferSource buffers, int light) {
        var model = loaded.get("shadow_brute");
        ...
        var motion=motions.computeIfAbsent(entity,e->new NativeShadowBrutePlayback(model));
