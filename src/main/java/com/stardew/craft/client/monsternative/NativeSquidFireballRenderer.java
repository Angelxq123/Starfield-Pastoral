...
import com.stardew.craft.entity.projectile.SquidFireballEntity;
...
public final class NativeSquidFireballRenderer extends EntityRenderer<SquidFireballEntity>{
    ...
    @Override public void render(SquidFireballEntity e,float yaw,float p,PoseStack stack,MultiBufferSource buffers,int light){
        ...
        for(var d:dots){var offset=d.pos.subtract(e.position());stack.pushPose();stack.translate(offset.x,offset.y+e.getBbHeight()/2,offset.z);stack.scale(1F/16,1F/16,1F/16);draw(stack,buffers,d.alpha);stack.popPose();}
