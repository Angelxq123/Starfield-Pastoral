...
public final class WoodWeaponVisuals {
    private record Effect(WoodWeaponFxPayload data,long born) {}
    ...
    public static void receive(WoodWeaponFxPayload p) {
        ...
        if(p.target()<0)level.playLocalSound(p.center().x,p.center().y+.4,p.center().z,SoundEvents.WOOD_HIT,SoundSource.PLAYERS,p.phase()==0?.6f:.85f,p.phase()==0?.9f:.6f,false);
            ...
            if(p.caster()==mc.player.getId()){CameraShakeState.kick(p.phase()==3?.045f:.10f,p.phase()==3?2:3,0);
                MeleeWeaponVisuals.authoredHeavyContact(p.skill(),p.phase()==3?.45f:.85f);
            }
        }
    }
    ...
    @SubscribeEvent public static void render(RenderLevelStageEvent e) {
        ...
        for(int pass=0;pass<2;pass++) {
            ...
            for(Effect effect:EFFECTS) {
                ...
                if(p.target()<0)WoodWeaponGeometry.draw(out,pose,p.center(),p.phase(),age,p.yaw(),edge);
                else {
                    ...
                    float fade=(1-age/8)*(1-age/8);int r=edge?40:246,g=edge?28:222,b=edge?17:175;
                    if(p.phase()==3)WeaponContactGeometry.slenderCross(out,pose,point,right,up,normal,.58f,fade,edge,r,g,b);
                    else WeaponContactGeometry.fourDiamonds(out,pose,point,right,up,normal,p.phase()==5?1.1f:.9f,fade,edge,r,g,b);
                }
            }
