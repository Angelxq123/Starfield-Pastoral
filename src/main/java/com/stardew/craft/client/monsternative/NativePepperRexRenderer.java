...
/** Native olive Rex, articulated jaw and rigid feet; no Geckolib or egg item. */
@SuppressWarnings({"null", "removal"})
@EventBusSubscriber(modid = StardewCraft.MODID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
public final class NativePepperRexRenderer extends EntityRenderer<MinePepperRexEntity> {
    private static final String[] VARIANTS = {"pepper_rex"};
    private static volatile java.util.Map<String, NativeNpcModel> loaded = java.util.Map.of();
    private NativeNpcModel posedModel;
    private final java.util.Map<MinePepperRexEntity,NativePepperRexPlayback> motions=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativePepperRexRenderer(EntityRendererProvider.Context context) { super(context); shadowRadius = .55F; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.PEPPER_REX.get(), NativePepperRexRenderer::new);
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativePepperRexRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native Pepper Rex model header");
            }
            ...
            for (String suffix : new String[]{"idle","walk","prepare","fire","hit","death"}) {
                var clip = model.clips().get("animation.pepper_rex." + suffix);
    ...
    @Override protected boolean shouldShowName(MinePepperRexEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(MinePepperRexEntity entity) { return texture("pepper_rex"); }
    @Override public void render(MinePepperRexEntity entity, float yaw, float partialTick, PoseStack stack,
                                 MultiBufferSource buffers, int light) {
        var model = loaded.get("pepper_rex");
        if (model == null || entity.isInvisible() || entity.deathTime+partialTick>=9) return;
        if (posedModel != model) { posedModel=model;motions.clear(); }
        var motion=motions.computeIfAbsent(entity,e->new NativePepperRexPlayback(model));
        motion.sample((entity.level().getGameTime()+(double)partialTick)/20.,entity.moving(),entity.phase(),entity.animationTime(partialTick),entity.hitTime(partialTick),
        ...
        var consumer=buffers.getBuffer(entity.deathTime>0?RenderType.entityTranslucentCull(texture("pepper_rex")):RenderType.entityCutout(texture("pepper_rex")));
        int alpha=entity.deathTime>0?Math.round(255*(1-(entity.deathTime+partialTick)/9)):255;
        for(var quad:model.quads()){
