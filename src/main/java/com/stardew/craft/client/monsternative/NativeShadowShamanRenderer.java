...
public final class NativeShadowShamanRenderer extends EntityRenderer<MineShadowShamanEntity> {
    private static final String[] VARIANTS = {"shadow_shaman"};
    private static volatile java.util.Map<String, NativeNpcModel> loaded = java.util.Map.of();
    private NativeNpcModel posedModel;
    private final java.util.Map<MineShadowShamanEntity,NativeShadowShamanPlayback> motions=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativeShadowShamanRenderer(EntityRendererProvider.Context context) { super(context); shadowRadius = .28F; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.SHADOW_SHAMAN.get(), NativeShadowShamanRenderer::new);
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativeShadowShamanRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native shadow shaman model header");
            }
            ...
            for (String suffix : new String[]{"idle","walk","cast","recover","hit","death"}) {
                var clip = model.clips().get("animation.shadow_shaman." + suffix);
    ...
    @Override protected boolean shouldShowName(MineShadowShamanEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(MineShadowShamanEntity entity) { return texture("shadow_shaman"); }
    @Override public void render(MineShadowShamanEntity entity, float yaw, float partialTick, PoseStack stack,
                                 MultiBufferSource buffers, int light) {
        var model = loaded.get("shadow_shaman");
        ...
        var motion=motions.computeIfAbsent(entity,e->new NativeShadowShamanPlayback(model));
        motion.sample((entity.level().getGameTime()+(double)partialTick)/20.,entity.moving(),entity.casting(),entity.castTime(partialTick),entity.releaseTime(partialTick),
        ...
        stack.popPose();
        if(entity.casting()&&entity.deathTime==0){
            double phase=(((entity.level().getGameTime()%20)*50+partialTick*50)%1000)*Math.PI/128/24;
            for(int i=0;i<8;i++){double a=phase+i*Math.PI/4;stack.pushPose();stack.translate(Math.cos(a)*4.4,1.05,Math.sin(a)*4.4);stack.scale(.9F/16,.9F/16,.9F/16);NativeShamanCurseRenderer.draw(stack,buffers,.7F);stack.popPose();}
        }
