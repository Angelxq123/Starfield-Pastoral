...
public final class NativeSkeletonRenderer extends EntityRenderer<MineSkeletonEntity> {
    private static final String[] VARIANTS = {"skeleton","skeleton_bone"};
    private static volatile java.util.Map<String, NativeNpcModel> loaded = java.util.Map.of();
    private NativeNpcModel posedModel;
    private final java.util.Map<MineSkeletonEntity,NativeSkeletonPlayback> motions=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativeSkeletonRenderer(EntityRendererProvider.Context context) { super(context); shadowRadius = .28F; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.SKELETON.get(), NativeSkeletonRenderer::new);
        event.registerEntityRenderer(ModEntities.SKELETON_BONE.get(), NativeSkeletonBoneRenderer::new);
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativeSkeletonRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native skeleton model header");
            }
            ...
            for (String suffix : variant.equals("skeleton_bone") ? new String[]{} : new String[]{"idle","walk","throw","recover","hit","death"}) {
                var clip = model.clips().get("animation.skeleton." + suffix);
    ...
    @Override protected boolean shouldShowName(MineSkeletonEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(MineSkeletonEntity entity) { return texture("skeleton"); }
    @Override public void render(MineSkeletonEntity entity, float yaw, float partialTick, PoseStack stack,
                                 MultiBufferSource buffers, int light) {
        var model = loaded.get("skeleton");
        ...
        var motion=motions.computeIfAbsent(entity,e->new NativeSkeletonPlayback(model));
        motion.sample((entity.level().getGameTime()+(double)partialTick)/20.,entity.moving(),entity.phase(),entity.throwProgress(partialTick),
                entity.releaseTime(partialTick),entity.hitTime(partialTick),entity.deathTime>0?(entity.deathTime+partialTick)/20.:0);
