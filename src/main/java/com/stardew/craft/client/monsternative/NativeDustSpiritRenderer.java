...
public final class NativeDustSpiritRenderer extends EntityRenderer<MineDustSpiritEntity> {
    private static final String[] VARIANTS = {"dust_spirit"};
    private static volatile java.util.Map<String, NativeNpcModel> loaded = java.util.Map.of();
    private NativeNpcModel posedModel;
    private final java.util.Map<MineDustSpiritEntity,NativeDustSpiritPlayback> motions=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativeDustSpiritRenderer(EntityRendererProvider.Context context) { super(context); shadowRadius = .28F; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.DUST_SPIRIT.get(), NativeDustSpiritRenderer::new);
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativeDustSpiritRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native dust_spirit model header");
            }
            ...
            for (String suffix : new String[]{"hop","flutter","hit","death"}) {
                var clip = model.clips().get("animation.dust_spirit." + suffix);
    ...
    @Override protected boolean shouldShowName(MineDustSpiritEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(MineDustSpiritEntity entity) { return texture("dust_spirit"); }
    @Override public void render(MineDustSpiritEntity entity, float yaw, float partialTick, PoseStack stack,
                                 MultiBufferSource buffers, int light) {
        var model = loaded.get("dust_spirit");
        if (model == null || entity.isInvisible()) return;
        if (posedModel != model) { posedModel=model;motions.clear(); }
        var motion=motions.computeIfAbsent(entity,e->new NativeDustSpiritPlayback(model));
        motion.sample((entity.level().getGameTime()+(double)partialTick)/20.,entity.sourceOffset(),entity.getScale(),entity.hitTime(partialTick),
