...
public final class OrdinaryMinePopulation {
    ...
    private record PlannedMonster(int x, int z, String species, boolean special, int dustConstructor, int facing) {}
    private final List<PlannedMonster> plannedMonsters = new ArrayList<>();
    ...
    public void populate() {
        ...
        populationLimit = monsterChance > 0
                ? MineGenerationBalance.normalizeMonsterCount(floor, Integer.MAX_VALUE, musk && (!garlic || floor>120)) : 0;
        edgeBarrels();
        ...
        prepareMonsterPopulation(monsterChance > 0, musk && (!garlic || floor>120));
        MineFloorDataManager.get(level).setFloorData(floor,data);
    }
    private void prepareMonsterPopulation(boolean enabled, boolean musk) {
        if (!enabled) return;
        int target = MineGenerationBalance.normalizeMonsterCount(floor, plannedMonsters.size(), musk);
        var candidates = new ArrayList<>(layout.cells);
        candidates.sort(Comparator.comparingDouble(c -> layout.distanceFromEntry(c.x(), c.z())));
        for (var cell : candidates) {
            if (plannedMonsters.size() >= target) break;
            if (layout.distanceFromEntry(cell.x(), cell.z()) > 5 && clear(cell.x(), cell.z(), false))
                spawn(cell.x(), cell.z(), chooseMonster(cell.x(), cell.z()));
        }
        com.stardew.craft.time.StardewSimulationTaskScheduler.schedule(level,
                INITIAL_MONSTER_SPAWN_DELAY_TICKS, () -> submitMonsterPopulation(List.copyOf(plannedMonsters), true));
    }
    private void submitMonsterPopulation(List<PlannedMonster> pending, boolean retry) {
        // A rebuilt/expired floor must never receive a delayed population from its old generation.
        if (MineFloorDataManager.get(level).getFloorData(floor) != data) return;
        var rejected = new ArrayList<PlannedMonster>();
        for (var plan : pending) {
            BlockPos position = pos(plan.x(), plan.z());
            if (!level.getBlockState(position).isAir() || !level.getBlockState(position.above()).isAir()) continue;
            var mob = MineMonsterSpawnHandler.spawnConfiguredMonster(level, plan.species(),
                    Vec3.atBottomCenterOf(position), plan.species().equals("bug") ? plan.facing()*90-180 : 0,
                    com.stardew.craft.monster.MonsterSpawnContext.mine(level,floor,data), m -> {
                        if (m instanceof com.stardew.craft.entity.monster.MineDustSpiritEntity dust && plan.dustConstructor() >= 0)
                            dust.sourceChargingConstructor(plan.dustConstructor() == 1);
                        m.setPersistenceRequired(); m.addTag(OrdinaryMineRuntime.MOB_TAG);
                        if (plan.special()) m.addTag(OrdinaryMineSpecialLoot.TAG);
                    });
            if (mob != null) data.addGeneratedMonster(mob.getUUID());
            else rejected.add(plan);
        }
        MineFloorDataManager.get(level).setFloorData(floor,data);
        if (com.stardew.craft.Config.isServerDebugLoggingEnabled())
            com.stardew.craft.StardewCraft.LOGGER.info("[MINE_DEBUG] population floor={} planned={} alive={} rejected={} retry={}",
                    floor, pending.size(), data.getEnemyCount(), rejected.size(), retry);
        // Retry only rejected insertions once; killed monsters are never resurrected on re-entry.
        if (retry && !rejected.isEmpty())
            com.stardew.craft.time.StardewSimulationTaskScheduler.schedule(level,
                    INITIAL_MONSTER_SPAWN_DELAY_TICKS, () -> submitMonsterPopulation(rejected, false));
    }
    ...
    private void spawn(int x,int z,String monster,boolean special) {
        if (plannedMonsters.size() >= populationLimit) return;
        plannedMonsters.add(new PlannedMonster(x,z,monster,special,selectedDustConstructor,bugFacing));
        occupied.add(pos(x,z));
    }
