#!/usr/bin/env python3
"""Import the authored forest farm schematic with audited coordinates."""
...
def main():
    ...
    parser.add_argument('--source', type=Path,
                        default=Path('run/config/worldedit/schematics/farm_3.schem'))
    parser.add_argument('--source-dir', type=Path,
                        default=Path('run/config/worldedit/schematics'))
    parser.add_argument('--output', type=Path,
                        default=Path('src/main/resources/data/stardewcraft/structures/farm/forest.schem'))
    parser.add_argument('--report', type=Path,
                        default=Path('scripts/data/forest_farm_report.json'))
    args = parser.parse_args()

    intake = load_intake(Path(__file__).resolve().parent)
    farm = intake.Schematic(args.source)
    expected_size = (288, 98, 272)
    if farm.size != expected_size:
        raise ValueError(f'farm_3 size changed: {farm.size} != {expected_size}')
    ...
    checks = {
        ...
        'spawn_ground': ([199, 24, 93], 'stardewcraft:dark_grass_block[snowy=false]'),
        'pet_bowl': ([178, 25, 70], 'minecraft:air'),
        'pet_bowl_ground': ([178, 24, 70], 'stardewcraft:dirt[variant=0]'),
        'totem': ([218, 25, 74], 'minecraft:air'),
        'totem_ground': ([218, 24, 74], 'stardewcraft:grass_block[snowy=false,variant=0]'),
        'cave_wall': ([125, 26, 72], 'minecraft:black_concrete'),
        'cave_portal': ([125, 25, 73], 'minecraft:air'),
    ...
    stump_sites = [
        [76, 25, 113], [87, 25, 114], [80, 25, 117],
        [89, 25, 126], [81, 25, 131], [85, 25, 141],
        [79, 25, 162], [92, 25, 168], [83, 25, 169],
        [98, 25, 180], [87, 25, 181], [81, 25, 186],
    ]
    for position in stump_sites:
        require_state(
            farm,
            position,
            'stardewcraft:large_stump[facing=south,part=main]')
    ...
    report = {
        ...
        'renewable_hardwood_stump_sites': stump_sites,
    ...
    print(f'Imported forest farm {farm.size} -> {imported.size}')
