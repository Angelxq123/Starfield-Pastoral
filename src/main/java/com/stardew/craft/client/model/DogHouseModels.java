...
public final class DogHouseModels {
    ...
    public static void register(ModelEvent.RegisterAdditional event) {
        for (String season : SEASONS) { event.register(id(season, "full")); event.register(id(season, "cell_0")); }
    }

    @SubscribeEvent
    public static void bake(ModelEvent.ModifyBakingResult event) {
        TextureAtlasSprite[][] materials = new TextureAtlasSprite[4][3];
        for (int s = 0; s < 4; s++) {
    ...
    private static final class Seasonal extends BakedModelWrapper<BakedModel> implements IDynamicBakedModel {
        ...
        Seasonal(BakedModel original, TextureAtlasSprite[][] materials, boolean bowl) {
            super(original); this.materials = materials; this.particleIndex = bowl ? 2 : 1;
        ...
        @Override public TextureAtlasSprite getParticleIcon() { return materials[TerrainSeasonTextures.currentTextureSet()][particleIndex]; }
