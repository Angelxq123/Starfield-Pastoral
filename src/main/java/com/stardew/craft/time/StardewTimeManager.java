...
public class StardewTimeManager extends SavedData {
    ...
    public void advanceDayWithSleepTime(int sleepMinute) {
        ...
        ServerLevel stardewLevel = server.getLevel(
                com.stardew.craft.core.ModDimensions.STARDEW_VALLEY);
        String previousWeather = stardewLevel == null
                ? "Sun"
                : com.stardew.craft.weather.WeatherManager.getCurrentWeather(stardewLevel);
        ...
        java.util.Set<java.util.UUID> frozenOnlinePlayerIds =
                java.util.Set.copyOf(allOnlinePlayerIds);
        int dayBeingSettled = com.stardew.craft.farm.OfflineFarmCatchUp.computeAbsoluteDay();
        for (com.stardew.craft.farm.FarmInstance farm : farms.getAllFarms()) {
            if (com.stardew.craft.farm.FarmDailyProcessHelper.shouldSettleFarm(
                    farm, frozenOnlinePlayerIds, dayBeingSettled)) {
