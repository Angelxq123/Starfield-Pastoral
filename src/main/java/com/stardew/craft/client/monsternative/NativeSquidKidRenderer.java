...
/** Source-shaped dark silhouette and hooked arms, rendered with the shared native cuboid format. */
@SuppressWarnings({"null", "removal"})
@EventBusSubscriber(modid = StardewCraft.MODID, bus = EventBusSubscriber.Bus.MOD, value = Dist.CLIENT)
public final class NativeSquidKidRenderer extends EntityRenderer<MineSquidKidEntity> {
    private static final String[] VARIANTS = {"squid_kid"};
    private static volatile java.util.Map<String, NativeNpcModel> loaded = java.util.Map.of();
    private NativeNpcModel posedModel;
    private final java.util.Map<MineSquidKidEntity,NativeSquidKidPlayback> motions=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativeSquidKidRenderer(EntityRendererProvider.Context context) { super(context); shadowRadius = .28F; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.SQUID_KID.get(), NativeSquidKidRenderer::new);
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativeSquidKidRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native squid kid model header");
            }
            ...
            for (String suffix : new String[]{"hover","fire","hit","death"}) {
                var clip = model.clips().get("animation.squid_kid." + suffix);
            ...
            for(var suffix:new String[]{"_blink","_fire","_hit"})resources.getResourceOrThrow(texture(variant+suffix));
            return model;
        } catch (Exception ex) {
    ...
    @Override protected boolean shouldShowName(MineSquidKidEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(MineSquidKidEntity entity) { return texture("squid_kid"+(entity.expression()==1?"_blink":entity.expression()==3?(entity.hitTime(0)<.2?"_hit":"_fire"):"")); }
    @Override public void render(MineSquidKidEntity entity, float yaw, float partialTick, PoseStack stack,
                                 MultiBufferSource buffers, int light) {
        var model = loaded.get("squid_kid");
        if (model == null || entity.isInvisible() || entity.deathTime+partialTick>=9.8) return;
        if (posedModel != model) { posedModel=model;motions.clear(); }
        var motion=motions.computeIfAbsent(entity,e->new NativeSquidKidPlayback(model));
        motion.sample((entity.level().getGameTime()+(double)partialTick)/20.,entity.fireTime(partialTick),
        ...
        var consumer=buffers.getBuffer(entity.expression()==1?NativeGhostRenderTypes.SQUID_BLINK:entity.expression()==3?(entity.hitTime(0)<.2?NativeGhostRenderTypes.SQUID_HIT:NativeGhostRenderTypes.SQUID_FIRE):NativeGhostRenderTypes.SQUID);
        for(var quad:model.quads()){
            ...
            for(var v:quad.vertices()){
                ...
                consumer.addVertex(stack.last().pose(),vertex.x,vertex.y,vertex.z).setColor(255,255,255,entity.deathTime>0?(int)(255*Math.max(0,1-(entity.deathTime+partialTick)/9.8)):255).setUv(v[3],v[4])
