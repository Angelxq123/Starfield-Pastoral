...
import com.stardew.craft.entity.monster.MineGrubEntity;
...
public final class NativeInsectRenderer extends EntityRenderer<StardewMonsterEntity> {
    private static final String[] VARIANTS = {"grub","fly"};
    ...
    private final java.util.Map<StardewMonsterEntity,NativeGrubPlayback> grubs=new java.util.WeakHashMap<>();
    private final java.util.Map<StardewMonsterEntity,NativeFlyPlayback> flies=new java.util.WeakHashMap<>();
    private final Vector3f vertex = new Vector3f(), normal = new Vector3f();
    private final Matrix3f normalMatrix = new Matrix3f();
    public NativeInsectRenderer(EntityRendererProvider.Context context,String variant) { super(context);this.variant=variant;shadowRadius = 0; }
    @SubscribeEvent public static void register(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.GRUB.get(), c->new NativeInsectRenderer(c,"grub"));
        event.registerEntityRenderer(ModEntities.FLY.get(), c->new NativeInsectRenderer(c,"fly"));
    }
    @SubscribeEvent public static void reload(RegisterClientReloadListenersEvent event) {
        event.registerReloadListener((ResourceManagerReloadListener) NativeInsectRenderer::load);
    }
    ...
    private static NativeNpcModel loadVariant(ResourceManager resources, String variant) {
        ...
        try (var reader = resources.getResourceOrThrow(modelId).openAsReader()) {
            ...
            if (model == null || model.version() != 1 || model.bones() == null || model.bones().isEmpty()
                ...
                throw new IllegalArgumentException("Invalid native insect model header");
            }
            ...
            for (String suffix : variant.equals("grub")?new String[]{"idle","crawl","form","pupa","hit","death"}:new String[]{"fly","spawn","hit","death"}) {
                var clip = model.clips().get("animation."+variant+"." + suffix);
            ...
            if(variant.equals("grub"))for(int i=0;i<4;i++)resources.getResourceOrThrow(texture("grub_form_"+i));
            return model;
        } catch (Exception ex) {
    ...
    @Override protected boolean shouldShowName(StardewMonsterEntity entity) { return false; }
    ...
    @Override public ResourceLocation getTextureLocation(StardewMonsterEntity entity) { return texture(entity instanceof MineGrubEntity g&&g.phase()>=2?"grub_form_"+(g.phase()==3?3:Math.min(3,(int)(g.formProgress()*3))):variant); }
    @Override public void render(StardewMonsterEntity entity, float yaw, float partialTick, PoseStack stack,
                                 MultiBufferSource buffers, int light) {
        var model = loaded.get(variant);
        if (model == null || entity.isInvisible()) return;
        if (posedModel != model) { posedModel=model;grubs.clear();flies.clear(); }
        NativeNpcPose pose;
        double clock=(entity.level().getGameTime()+(double)partialTick)/20.;
        double death=entity.deathTime>0?(entity.deathTime+partialTick)/20.:0;
        if(entity instanceof MineGrubEntity grub) {
            var motion=grubs.computeIfAbsent(entity,e->new NativeGrubPlayback(model));
            motion.sample(clock,grub.phase(),grub.moving(),grub.animationTime(partialTick),grub.formProgress(),grub.hitTime(partialTick),death);pose=motion.pose();
        } else {
            var fly=(MineFlyEntity)entity;
            var motion=flies.computeIfAbsent(entity,e->new NativeFlyPlayback(model));
            motion.sample(clock,fly.spawnTime(partialTick),fly.hitTime(partialTick),death);pose=motion.pose();
        }
        ...
        for (boolean translucent : new boolean[]{false,true}) {
            var consumer = buffers.getBuffer(translucent?NativeFlyRenderTypes.MEMBRANE:RenderType.entityCutout(getTextureLocation(entity)));
            for (var quad : model.quads()) {
