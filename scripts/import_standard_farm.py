#!/usr/bin/env python3
"""Import the authored standard farm schematic with audited coordinates."""
...
def main():
    ...
    parser.add_argument('--source', type=Path,
                        default=Path('run/config/worldedit/schematics/farm_1.schem'))
    parser.add_argument('--source-dir', type=Path,
                        default=Path('run/config/worldedit/schematics'))
    parser.add_argument('--output', type=Path,
                        default=Path('src/main/resources/data/stardewcraft/structures/farm/standard.schem'))
    parser.add_argument('--report', type=Path,
                        default=Path('scripts/data/standard_farm_report.json'))
    args = parser.parse_args()

    intake = load_intake(Path(__file__).resolve().parent)
    farm = intake.Schematic(args.source)
    expected_size = (288, 108, 272)
    if farm.size != expected_size:
        raise ValueError(f'farm_1 size changed: {farm.size} != {expected_size}')
    ...
    checks = {
        'spawn': ([199, 25, 94], 'minecraft:air'),
        'spawn_ground': ([199, 24, 94], 'stardewcraft:grass_block[snowy=false,variant=0]'),
        'pet_bowl': ([183, 25, 77], 'minecraft:air'),
        'pet_bowl_ground': ([183, 24, 77], 'stardewcraft:grass_block[snowy=false,variant=0]'),
        'totem': ([175, 25, 71], 'minecraft:air'),
        'cave_wall': ([129, 26, 70], 'minecraft:black_concrete'),
        'cave_portal': ([129, 25, 71], 'minecraft:air'),
        'north_exit': ([142, 25, 36], 'minecraft:air'),
    ...
    print(f'Imported standard farm {farm.size} -> {imported.size}')
